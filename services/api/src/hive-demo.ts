// Hive demo - showcases full agent flow on X
...
const DEMO_THREAD: DemoStep[] = [
  {
    content: `Hive demo thread - ZK-private agent coordination on Solana.
...
export async function runHiveDemo(twitter: TwitterApi): Promise<string[]> {
  const tweetIds: string[] = [];
  let lastTweetId: string | undefined;

  logger.info('Starting Hive demo thread...');
  ...
  logger.info('Hive demo thread complete', { tweetCount: tweetIds.length });
  return tweetIds;
}
